...
import gleamunison/types.{
  ArityMismatch, CTAbility, ExtraOperation, MissingOperation, OperationType,
  TypeCache, validate_handler,
}
...
pub fn handler_missing_op_test() {
  ...
  let cache =
    TypeCache(entries: dict.from_list([#(ability_ref, CTAbility([op]))]))

  let assert Error(MissingOperation(ref, "read", 0)) =
    validate_handler(cache, ability_ref, dict.new())
...
pub fn handler_extra_op_test() {
  ...
  let assert Error(ExtraOperation(ref, 5)) =
    validate_handler(cache, ability_ref, handler_ops)
  let assert True = ref == ability_ref
}
