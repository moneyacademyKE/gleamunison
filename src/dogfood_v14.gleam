...
pub fn level1602() -> Nil {
  io.println("--- Level 1602: Guard with undefined variable propagates error ---")
  ...
  case elaborate_term(
    ...
    Ok(_) -> io.println("Note: guard error now propagates (was swallowed before fix)")
...
pub fn level1603() -> Nil {
  ...
  let surf = elab_types.SurfaceUnit(
    Ref(hash_bytes(bit_array.from_string("guard_fix_v14"))),
    [#("test_guard", elab_types.SurfaceTermDef(
      elab_types.SMatch(elab_types.SInt(42), [
        elab_types.SCase(
          pattern: elab_types.SPVar("x"),
...
pub fn level1604() -> Nil {
  ...
  let h1: fn(List(Dynamic), fn(Dynamic) -> Dynamic) -> Dynamic =
    fn(_a, c) { c(ffi_to_dynamic(10)) }
  let h2: fn(List(Dynamic), fn(Dynamic) -> Dynamic) -> Dynamic =
    fn(_a, c) { c(ffi_to_dynamic(20)) }
...
pub fn level1607() -> Nil {
  ...
  case elaborate_pattern(pat, ctx) {
    Ok(#(_, elaborated)) -> io.println("PatCons: " <> string.inspect(elaborated))
    Error(e) -> io.println("Error: " <> string.inspect(e))
  }
...
pub fn level1608() -> Nil {
  ...
  case parse_string("(match (pair (pair 1 2) 3) ((pair (pair a b) c) (add a (add b c))))") {
    Ok(term) -> {
...
pub fn level1609() -> Nil {
  ...
  let ctx = empty_elab_ctx()
  case elaborate_pattern(pat, ctx) {
    Ok(#(_, elaborated)) -> io.println("As+Cons: " <> string.inspect(elaborated))
    Error(e) -> io.println("Error: " <> string.inspect(e))
  }
...
pub fn level1610() -> Nil {
  ...
  let inner_match = ast.Match(ast.LocalVarRef(Local(0)), [
  ...
  let outer_match = ast.Match(ast.LocalVarRef(Local(0)), [
  ...
  case compile_definition(new_compiler(), d, r) {
    Ok(b) -> io.println("Deep Let+Match: " <> int.to_string(bit_array.byte_size(b)) <> " bytes")
    Error(e) -> io.println("Error: " <> string.inspect(e))
  }
...
pub fn level1611() -> Nil {
  ...
  let inner = ast.Lambda(Local(1), ast.Apply(ast.LocalVarRef(Local(0)), ast.LocalVarRef(Local(1))))
  let outer = ast.Lambda(Local(0), ast.Apply(ast.LocalVarRef(Local(0)), inner))
  let d = ast.TermDef(outer, ast.Builtin(ast.IntType))
  let r = Ref(hash_of_definition(d))
  case compile_definition(new_compiler(), d, r) {
    Ok(b) -> io.println("Closure-of-closure: " <> int.to_string(bit_array.byte_size(b)) <> " bytes")
    Error(e) -> io.println("Error: " <> string.inspect(e))
  }
...
pub fn level1612() -> Nil {
  ...
  let r = Ref(hash_of_definition(d))
  case compile_definition(new_compiler(), d, r) {
    Ok(b) -> io.println("PatConstructor guard: " <> int.to_string(bit_array.byte_size(b)) <> " bytes")
    Error(e) -> io.println("Error: " <> string.inspect(e))
  }
...
pub fn level1619() -> Nil {
  ...
  case library_eval("(list-length (list-filter (lam x (gt? x 5)) (list-map (lam x (add x 3)) (list 1 2 3 4 5))))") {
...
pub fn level1621() -> Nil {
  ...
  case library_eval("(let fact (lam n (match n (0 1) (_ (mul n (fact (sub n 1)))))) (fact 7))") {
  ...
  let _ = case storage.mnesia("v14_mnesia_bulk") {
    Ok(adapter) -> {
      ...
      case adapter.list_refs() {
        Ok(refs) -> io.println("Mnesia bulk: " <> int.to_string(list.length(refs)) <> " refs")
        Error(e) -> io.println("Error: " <> string.inspect(e))
      }
      let _ = adapter.close()
...
pub fn level1623() -> Nil {
  ...
  let cache = types.TypeCache(entries: dict.from_list([
    #(ab_ref, types.CTAbility([
      types.OperationType(name: Some("run"), inputs: [ast.Builtin(ast.IntType)], output: ast.Builtin(ast.IntType)),
    ])),
  ]))
...
pub fn level1628() -> Nil {
  ...
  start_server(18502)
  let _ = get("http://localhost:18502/define?name=test_func&expr=(lam%20x%20(add%20x%201))")
...
pub fn level1632() -> Nil {
  ...
  case library_eval("((lam f (lam g (f (g 6)))) (lam x (add x 2)) (lam y (mul y 3)))") {
...
pub fn level1633() -> Nil {
  ...
  case library_eval("(let s (string-concat \"hello\" \"world\") (let len (string-length s) (let upper (string-upcase s) len)))") {
...
pub fn level1635() -> Nil {
  ...
  case library_eval("(list-fold (lam acc (lam x (mul acc x))) 1 (list 2 3 4))") {
...
pub fn level1637() -> Nil {
  ...
  case adapter.lookup(ref) {
    Ok(Some(bytes)) -> io.println("Roundtrip: " <> int.to_string(bit_array.byte_size(bytes)) <> " bytes")
...
pub fn level1638() -> Nil {
  ...
  case repl_eval.handle_define("v1", elab_types.SInt(1), cache, prev) {
    Ok(#(c2, d1)) -> case repl_eval.handle_define("v2", elab_types.SInt(2), c2, d1) {
      Ok(#(c3, d2)) -> case repl_eval.handle_define("v3", elab_types.SInt(3), c3, d2) {
        Ok(#(_, d3)) -> io.println(int.to_string(list.length(d3)) <> " defs after 3 defines")
...
pub fn level1639() -> Nil {
  ...
  let ad = ast.AbilityDecl(ast.AbilityDeclaration(Local(0), [
    ...
    ast.Operation(Local(1), [ast.TypeRefBuiltin(ast.TextType)], ast.TypeRefBuiltin(ast.TextType)),
    ast.Operation(Local(2), [ast.TypeRefBuiltin(ast.IntType), ast.TypeRefBuiltin(ast.IntType)], ast.TypeRefBuiltin(ast.IntType)),
  ]))
  let r = Ref(hash_of_definition(ast.TermDef(ast.Int(1), ast.Builtin(ast.IntType))))
  case compile_definition(new_compiler(), ad, r) {
    Ok(b) -> io.println("Multi-op AB: " <> int.to_string(bit_array.byte_size(b)) <> " bytes")
    Error(e) -> io.println("Error: " <> string.inspect(e))
  }
...
pub fn level1640() -> Nil {
  ...
  let adapter = codebase.get_adapter(cb)
  case adapter.list_refs() {
    Ok(refs) -> io.println("Mixed insert: " <> int.to_string(list.length(refs)) <> " refs")
    Error(e) -> io.println("Error: " <> string.inspect(e))
  }
...
pub fn level1641() -> Nil {
  ...
  case library_eval("(add (string-length \"hello\") (list-length (list-filter (lam x (eq? (mod x 2) 0)) (range 1 10))))") {
...
pub fn level1646() -> Nil {
  ...
  case library_eval("(let abs (lam n (match (lt? n 0) (1 (sub 0 n)) (0 n))) (abs -7))") {
...
pub fn level1647() -> Nil {
  ...
  case library_eval("(string-length (string-replace (string-upcase \"hello\") \"ELL\" \"XYZ\"))") {
...
pub fn level1648() -> Nil {
  ...
  case library_eval("(list-fold (lam acc (lam x (add acc x))) 0 (list-map (lam x (mul x x)) (range 1 5)))") {
...
pub fn level1649() -> Nil {
  ...
  io.println("  Guard fix (1601-1603): valid guard, error propagation, end-to-end verification")
  io.println("  Ability dispatch (1604-1606): dual handler chain, 4 distinct keys, duplicated handler")
  io.println("  Pattern depth (1607-1609): 3-level PatCons, nested constructor parse, deep As+Cons")
  io.println("  Compile edges (1610-1612): deep Let+Match, closure-of-closure, PatConstructor guard")
  io.println("  Lexer multi-line (1613-1614): actual newline, escaped multi-line")
  io.println("  Property failing (1615-1617): false property, range, trivially true")
  io.println("  Complex REPL (1618-1621): triple lambda, map+filter, closure-of-closure, fact(7)")
  io.println("  Storage+Typecheck (1622-1626): Mnesia bulk 100, Handle with ab ref, Do with cache, jet miss, cross-def AB+Term")
  io.println("  HTTP deeper (1627-1631): static serve, define+verify, eval via HTTP, 3 defines, status+health")
  io.println("  Complex eval (1632-1635): HOF chain, string let, string match, fold mul")
  io.println("  Storage+Codebase (1636-1640): 3000-insert, roundtrip, 3-define, multi-op AbilityDecl, mixed insert list_refs")
  io.println("  Integration (1641-1650): full builtin chain, compile+load+eval, log+counter+trace+encode, typecheck+loader+codebase, effects+lexer+parser, conditional, string ops, list transform, summary, cert")
...
pub fn level1650() -> Nil {
  ...
  io.println("  v14 (1601-1650): Guard fix verification, ability dispatch through stacked handlers, pattern depth, compile deep edges, lexer multi-line, property failing, complex REPL, Mnesia+typecheck+jet edges, HTTP deeper, storage+codebase stress, integration")
