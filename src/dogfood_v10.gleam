...
import gleamunison/http_client.{delete, get}
...
pub fn level1408() -> Nil {
  ...
  start_server(18_308)
  let id_str = case bit_array.to_string(id) {
    Ok(s) -> s
    _ -> "unknown"
  }
  ...
  case repl_eval.handle_define("v", elab_types.SInt(1), cache, prev) {
    Ok(#(cache2, defs1)) -> {
      case repl_eval.handle_define("v", elab_types.SInt(99), cache2, defs1) {
        Ok(#(_, defs2)) -> {
          io.println(
            "Re-defines: " <> int.to_string(list.length(defs2)) <> " defs",
          )
        }
...
pub fn level1428() -> Nil {
  ...
  case adapter.list_refs() {
    Ok(refs) ->
      io.println(
        "200 defs stored: " <> int.to_string(list.length(refs)) <> " refs",
      )
...
fn open_dets_and_insert(path: String) -> Nil {
  case dets(path) {
    Ok(adapter) -> {
      ...
      let assert Ok(Nil) =
        adapter.insert(ref, bit_array.from_string("v10_persist"))
      let _ = adapter.close()
...
fn bulk_insert_500(adapter: storage.StorageAdapter, n: Int) -> Nil {
  case n {
    ...
    _ -> {
      let r =
        Ref(hash_bytes(bit_array.from_string("bulk_v10_" <> int.to_string(n))))
      let assert Ok(Nil) =
        adapter.insert(r, bit_array.from_string("v10_" <> int.to_string(n)))
...
pub fn level1431() -> Nil {
  ...
  case
    elaborate_term(
      elab_types.SConstruct("Pair", [elab_types.SInt(1), elab_types.SInt(2)]),
      ctx2,
    )
  {
    Ok(#(_, term)) -> {
      case term {
        ast.Construct(r, terms) -> {
          ...
          io.println(
            "Construct: " <> int.to_string(list.length(terms)) <> " args OK",
          )
        }
...
pub fn level1435() -> Nil {
  ...
  case compile_definition(new_compiler(), d, r) {
    Ok(b) ->
      io.println(
        "Empty list: " <> int.to_string(bit_array.byte_size(b)) <> " bytes",
      )
...
pub fn level1436() -> Nil {
  ...
  let t =
    ast.Let(
      ...
      ast.Let(
        Local(1),
        ast.Int(2),
        ast.Let(Local(2), ast.Int(3), ast.LocalVarRef(Local(2))),
      ),
    )
  ...
  case compile_definition(new_compiler(), d, r) {
    Ok(b) ->
      io.println(
        "Nested Let: " <> int.to_string(bit_array.byte_size(b)) <> " bytes",
      )
    Error(e) -> io.println("Fail: " <> string.inspect(e))
  }
...
pub fn level1438() -> Nil {
  ...
  case compile_definition(new_compiler(), d, r) {
    Ok(b) ->
      io.println(
        "Empty match: " <> int.to_string(bit_array.byte_size(b)) <> " bytes",
      )
    Error(e) -> io.println("Fail: " <> string.inspect(e))
  }
...
pub fn level1449() -> Nil {
  ...
  io.println(
    "  HTTP routes (1401-1412): counter, browse, processes, sync-status, modules, logs, traces, traces/:id, redefinitions, /, path traversal, 404",
  )
  io.println(
    "  Normalize+substitute (1413-1415): AbilityVar identity, nested Fn, substitute App",
  )
  io.println(
    "  REPL error codes (1416-1420): E002 UnknownOperation, E003 MissingAbilityDecl, E004 InferFailed, E005 UnsupportedTypeRef, redefine shadow",
  )
  io.println(
    "  Lexer edges (1421-1423): empty string, comment at end, unicode identifier",
  )
  io.println(
    "  Parser edges (1424-1427): SPText pattern, Cons pattern, >100 nesting, define SList",
  )
  io.println(
    "  Codebase+Storage (1428-1430): 200-def stress, DETS reopen persistence, 500-insert lookup",
  )
  io.println(
    "  Elaboration deeper (1431-1434): SConstruct with args, binding shadow, multi-input op, SRef",
  )
  io.println(
    "  Compile edges (1435-1438): empty list, nested Let, TypeDef, empty Match",
  )
  io.println(
    "  Inference deeper (1439-1441): list_all_match heterogeneous, check_linearity Let, Do op bounds",
  )
  io.println(
    "  Sync+Jet+Property (1442-1444): multi-ref sync, jet miss random, property check",
  )
  io.println(
    "  Integration (1445-1450): full pipeline, soft purge, 7-module cross, sync+lex+parser+typecheck, summary, cert",
  )
