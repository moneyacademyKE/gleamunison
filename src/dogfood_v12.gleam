...
pub fn level1509() -> Nil {
  ...
  case
    library_eval("(list-fold (lam acc (lam x (add acc x))) 0 (list 1 2 3 4))")
  {
...
pub fn level1510() -> Nil {
  ...
  case
    library_eval(
      "(list-length (list-sort (lam a (lam b (lt? a b))) (list 5 2 4 1 3)))",
    )
  {
...
pub fn level1513() -> Nil {
  ...
  case
    library_eval("(dict-get (dict-set (dict-new) \"key\" \"val\") \"key\")")
  {
...
pub fn level1516() -> Nil {
  ...
  case
    library_eval(
      "(let fact (lam n (match n (0 1) (_ (mul n (fact (sub n 1)))))) (fact 5))",
    )
  {
...
pub fn level1518() -> Nil {
  ...
  case
    library_eval(
      "((lam f (lam g (lam x (f (g x))))) (lam x (add x 1)) (lam x (mul x 2)) 5)",
    )
  {
...
pub fn level1519() -> Nil {
  ...
  case
    library_eval(
      "(let sum-to (lam n (match n (0 0) (_ (add n (sum-to (sub n 1)))))) (sum-to 10))",
    )
  {
...
pub fn level1520() -> Nil {
  ...
  case
    library_eval(
      "(let even? (lam n (match n (0 1) (_ (odd? (sub n 1))))) (let odd? (lam n (match n (0 0) (_ (even? (sub n 1))))) (even? 4)))",
    )
  {
...
pub fn level1521() -> Nil {
  ...
  case compile_definition(new_compiler(), d, r) {
    Ok(b) ->
      io.println(
        "PatText match: " <> int.to_string(bit_array.byte_size(b)) <> " bytes",
      )
...
pub fn level1522() -> Nil {
  ...
  case compile_definition(new_compiler(), d, r) {
    Ok(b) ->
      io.println(
        "Unused var: " <> int.to_string(bit_array.byte_size(b)) <> " bytes",
      )
    Error(e) -> io.println("Compile error: " <> string.inspect(e))
  }
...
pub fn level1523() -> Nil {
  ...
  let c =
    ast.Case(ast.PatCons(Local(0), Local(1)), None, ast.LocalVarRef(Local(0)))
  ...
  case compile_definition(new_compiler(), d, r) {
    Ok(b) ->
      io.println(
        "PatCons: " <> int.to_string(bit_array.byte_size(b)) <> " bytes",
      )
    Error(e) -> io.println("Compile error: " <> string.inspect(e))
  }
...
pub fn level1524() -> Nil {
  ...
  case compile_definition(new_compiler(), d, r) {
    Ok(b) ->
      io.println(
        "PatEmptyList: " <> int.to_string(bit_array.byte_size(b)) <> " bytes",
      )
    Error(e) -> io.println("Compile error: " <> string.inspect(e))
  }
...
pub fn level1526() -> Nil {
  io.println(
    "--- Level 1526: Push sync adapter lookup filters missing refs ---",
  )
  ...
  let _ = case push_sync(state, peer, [missing_ref], adapter) {
    Ok(#(_, count)) ->
      io.println("Push count (0 expected): " <> int.to_string(count))
...
pub fn level1529() -> Nil {
  ...
  case repl_eval.handle_define("a", elab_types.SInt(10), cache, prev) {
    Ok(#(c2, d1)) ->
      case repl_eval.handle_define("b", elab_types.SInt(20), c2, d1) {
        Ok(#(c3, d2)) -> {
...
pub fn level1530() -> Nil {
  ...
  let handler: fn(List(Dynamic), fn(Dynamic) -> Dynamic) -> Dynamic = fn(
    ...
    io.println(
      "Handler called with " <> int.to_string(list.length(args)) <> " args",
    )
...
pub fn level1531() -> Nil {
  ...
  case
    library_eval(
      "(list-length (list-filter (lam x (gt? x 3)) (list-map (lam x (mul x 2)) (list 1 2 3 4 5))))",
    )
  {
  ...
  let _ = case storage.dets(path) {
    Ok(a) -> {
      ...
      let _ = case a.list_refs() {
        Ok(refs) ->
          io.println("DETS list_refs: " <> int.to_string(list.length(refs)))
...
pub fn level1536() -> Nil {
  ...
  case compile_definition(new_compiler(), d, r) {
    Ok(b) ->
      io.println(
        "All patterns: " <> int.to_string(bit_array.byte_size(b)) <> " bytes",
      )
    Error(e) -> io.println("Error: " <> string.inspect(e))
  }
...
pub fn level1539() -> Nil {
  ...
  case
    library_eval(
      "(let double (lam x (mul x 2)) (let keep-gt5 (lam x (gt? x 5)) (list-length (list-filter keep-gt5 (list-map double (list 1 2 3 4 5))))))",
    )
  {
...
pub fn level1540() -> Nil {
  ...
  case
    library_eval(
      "(and (string-contains? \"hello\" \"ell\") (eq? (string-length \"abc\") 3))",
    )
  {
            ...
            [#(ref, def), ..] -> {
              case pipeline.compile_only(def, ref) {
                Ok(beam) -> {
                  ...
                  let _ =
                    pipeline.load_and_eval(compile.module_name_for(ref), beam)
                  Nil
                }
...
pub fn level1547() -> Nil {
  ...
  case
    library_eval(
      "(list-length (list-reverse (list-sort (lam a (lam b (lt? a b))) (list 9 3 7 1 5 2 8 4 6))))",
    )
  {
...
pub fn level1548() -> Nil {
  ...
  case
    library_eval(
      "(let make-adder (lam n (lam x (add x n))) (let add5 (make-adder 5) (add5 10)))",
    )
  {
...
pub fn level1549() -> Nil {
  ...
  io.println(
    "  Remaining string builtins (1501-1505): downcase, replace, trim, string->int, split",
  )
  io.println(
    "  Remaining list builtins (1506-1511): append, member?, flatten, fold, sort, range",
  )
  io.println(
    "  Data structure builtins (1512-1515): left/right, dict, set, json-parse",
  )
  io.println(
    "  Recursion+HOF (1516-1520): factorial, apply-twice, compose, sum-to, mutual recursion",
  )
  io.println(
    "  Compile pattern edges (1521-1524): PatText, unused var, PatCons, PatEmptyList",
  )
  io.println(
    "  Guard+sync+codebase (1525-1529): guard elaboration, push adapter filter, idempotent insert, 1000-def stress, multiple define",
  )
  io.println(
    "  Effects+Inference (1530-1532): handler args, complex nested eval, multi-line lexer",
  )
  io.println(
    "  Loader+Storage (1533-1536): 10 modules, DETS list_refs, multi jet miss, all pattern types",
  )
  io.println(
    "  Builtin+Cross (1537-1540): multi-let, conditional, chain, string-bool combo",
  )
  io.println(
    "  Integration (1541-1550): parse+compile+eval, loader+compile+storage, effects+codebase+lexer, inference+log+counter, all numeric ops, nested comparisons, list stress, lambda capture, summary, cert",
  )
