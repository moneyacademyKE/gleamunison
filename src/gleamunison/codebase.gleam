...
fn hash_type_ref(tr: ast.TypeRef) -> Hash {
  case tr {
    ast.TypeRefVar(Local(i)) -> hash_bytes(str_to_bits("typerefvar:" <> int.to_string(i)))
    ast.TypeCon(Ref(h)) -> hash_bytes(bit_array.concat([str_to_bits("typecon:"), hash_to_binary(h)]))
...
fn hash_pattern(p: ast.Pattern) -> Hash {
  case p {
    ast.PatVar(Local(i)) -> hash_bytes(str_to_bits("patvar:" <> int.to_string(i)))
    ...
    ast.PatCons(head: Local(h), tail: Local(t)) ->
      hash_bytes(str_to_bits("patcons:" <> int.to_string(h) <> ":" <> int.to_string(t)))
...
fn hash_case(c: ast.Case) -> Hash {
  hash_bytes(bit_array.concat([str_to_bits("case:"), hash_to_binary(hash_pattern(c.pattern)), hash_to_binary(hash_term(c.body))]))
}
...
fn hash_term(term: ast.Term) -> Hash {
  case term {
    ...
    ast.LocalVarRef(Local(i)) -> hash_bytes(str_to_bits("localvar:" <> int.to_string(i)))
    ast.RefTo(Ref(h)) -> hash_bytes(bit_array.concat([str_to_bits("refto:"), hash_to_binary(h)]))
    ast.Match(scrutinee:, cases:) ->
      hash_bytes(bit_array.concat([str_to_bits("match:"), hash_to_binary(hash_term(scrutinee)), ..list.map(cases, fn(c) { hash_to_binary(hash_case(c)) })]))
...
fn hash_type(typ: ast.Type) -> Hash {
  case typ {
    ...
    ast.AbilityVar(i) -> hash_bytes(str_to_bits("abilityvar:" <> int.to_string(i)))
    ast.Fn(params:, result:, requires: _) ->
      hash_bytes(bit_array.concat([str_to_bits("fn:"), hash_to_binary(hash_type(result)), ..list.map(params, fn(p) { hash_to_binary(hash_type(p)) })]))
    ast.App(Ref(h), args:) ->
      hash_bytes(bit_array.concat([str_to_bits("app:"), hash_to_binary(h), ..list.map(args, fn(a) { hash_to_binary(hash_type(a)) })]))
  }
}
...
pub fn hash_of_definition(def: ast.Definition) -> Hash {
  case def {
    ast.TermDef(term:, typ:) ->
      hash_bytes(bit_array.concat([str_to_bits("termdef:"), hash_to_binary(hash_term(term)), hash_to_binary(hash_type(typ))]))
    ast.TypeDef(ast.Structural(Local(n), parameters:, constructors:)) -> {
      let h1 = hash_bytes(bit_array.concat([str_to_bits("typedef:structural:" <> int.to_string(n)), ..list.map(parameters, fn(param) {
        let Local(i) = param
        str_to_bits("p" <> int.to_string(i))
      })]))
      hash_bytes(bit_array.concat([hash_to_binary(h1), ..list.map(constructors, fn(c) { hash_to_binary(hash_constructor(c)) })]))
    }
    ast.TypeDef(ast.Unique(Local(n), guid:, parameters: _, constructors: _)) ->
      hash_bytes(str_to_bits("typedef:unique:" <> int.to_string(n) <> ":" <> guid))
  ...
  case hash_equal(computed, hash) {
    ...
    True ->
      case dict.get(codebase.seen, computed) {
        Ok(_existing) -> Ok(codebase)  // idempotent: same content-addressed content
        Error(_) -> {
