...
import gleamunison/filepath.{
  file_name, from_string, has_extension, join, parent, to_string,
}
...
import gleamunison/types.{
  type TypeCache, CTAbility, CTTerm, CTType, OperationType, TypeCache,
  empty_cache,
}
...
pub fn level1752() -> Nil {
  ...
  case status {
    ...
    Unhealthy(msg) ->
      io.println("Unhealthy (if Degraded not implemented): " <> msg)
  }
...
pub fn level1753() -> Nil {
  ...
  case render(tmpl, vars) {
    ...
    Error(TemplateError(reason)) ->
      io.println("Template error (expected): " <> reason)
  }
    ...
    Ok(ldr2) -> {
      ...
      case ensure_loaded(ldr2, ref_b, def_b) {
        Ok(ldr3) -> {
          ...
          io.println(
            "After second load: a="
            <> string.inspect(loaded_a2)
            <> " b="
            <> string.inspect(loaded_b),
          )
        }
      ...
      case ensure_loaded(ldr2, ref_b, def_b) {
        Ok(ldr3) ->
          case ensure_loaded(ldr3, ref_c, def_c) {
            ...
            Error(#(_, err)) ->
              io.println("Load c error: " <> string.inspect(err))
          }
        Error(#(_, err)) -> io.println("Load b error: " <> string.inspect(err))
      }
    Error(#(_, err)) -> io.println("Load a error: " <> string.inspect(err))
  ...
  case ensure_loaded(ldr, ref, def) {
    Ok(ldr2) -> {
      case ensure_loaded(ldr2, ref, def) {
        ...
        Error(#(_, err)) ->
          io.println("Second load error: " <> string.inspect(err))
      }
    }
...
pub fn level1758() -> Nil {
  ...
  let ldr = new_loader()
  let typedef =
    ast.TypeDef(ast.Structural(Local(0), [], [ast.Constructor(Local(1), [])]))
...
pub fn level1759() -> Nil {
  io.println(
    "--- Level 1759: Typecheck unit with TermDef + TypeDef + AbilityDecl ---",
  )
...
pub fn level1760() -> Nil {
  io.println(
    "--- Level 1760: Typecheck with TermDef referencing TypeDef via TypeCon ---",
  )
  let type_ref = hash_bytes(bit_array.from_string("typed_con_v17"))
  let typedef =
    ast.TypeDef(
      ast.Structural(Local(0), [], [
        ast.Constructor(Local(1), [ast.TypeRefVar(Local(0))]),
      ]),
    )
  let term =
    ast.TermDef(
  ...
  let term_ref =
    hash_of_definition(ast.TermDef(ast.Int(0), ast.Builtin(ast.IntType)))
  let unit =
    ast.Unit(Ref(term_ref), [
...
pub fn level1762() -> Nil {
  ...
  let vars =
    list.map(
      ["a", "b", "c", "d", "e", "f", "g", "h", "i", "j"],
      fn(k: String) -> #(String, String) { #(k, k <> "_val") },
    )
  case render(tmpl, vars) {
    Ok(result) ->
      io.println("10-var template: " <> string.slice(result, 0, 30) <> "...")
...
pub fn level1767() -> Nil {
  ...
  let nested =
    list.fold(range(1, 50), "0", fn(acc: String, _n: Int) -> String {
      "(cons 1 " <> acc <> ")"
    })
...
pub fn level1769() -> Nil {
  io.println(
    "--- Level 1769: Inference Match with different case body types ---",
  )
  let match_term =
    ast.Match(ast.Int(42), [
      ...
      ast.Case(
        pattern: ast.PatInt(99),
        guard: option.None,
        body: ast.Text(bit_array.from_string("x")),
      ),
    ])
  let cache = empty_cache()
  case infer_term(match_term, cache) {
    Ok(typ) ->
      io.println("Match with mixed types inferred: " <> string.inspect(typ))
...
pub fn level1771() -> Nil {
  ...
  let hetero_list =
    ast.List([ast.Int(1), ast.Text(bit_array.from_string("x")), ast.Int(3)])
  let cache = empty_cache()
...
pub fn level1772() -> Nil {
  ...
  let handle_term =
    ast.Handle(ast.Do(ab_ref, Local(0), [ast.Int(1)]), ast.Int(0), ab_ref)
  let cache = empty_cache()
...
pub fn level1773() -> Nil {
  ...
  case cb_insert(empty_codebase(), unit) {
    ...
    Error(e) ->
      io.println("HashMismatch error (expected): " <> string.inspect(e))
  }
...
pub fn level1775() -> Nil {
  io.println(
    "--- Level 1775: Codebase insert 3 definitions then re-insert idempotent ---",
  )
  let defs =
  ...
  let unit =
    ast.Unit(Ref(hash_bytes(bit_array.from_string("root_reinsert_v17"))), defs)
  case cb_insert(empty_codebase(), unit) {
    Ok(cb) -> {
...
pub fn level1780() -> Nil {
  ...
  let variants = [
    ...
    ast.TermDef(
      ast.Match(ast.Int(0), [
        ast.Case(pattern: ast.PatInt(0), guard: option.None, body: ast.Int(1)),
      ]),
      ast.Builtin(ast.IntType),
    ),
  ...
  let results =
    list.map(variants, fn(def) {
      let h = hash_of_definition(def)
      compile_definition(compiler, def, Ref(h))
    })
  let all_ok =
    list.fold(results, True, fn(acc, r) {
      case acc {
        False -> False
        True ->
          case r {
...
pub fn level1783() -> Nil {
  ...
  case json.decode(bad_json) {
    Ok(v) ->
      io.println("Decoded invalid JSON (unexpected): " <> string.inspect(v))
    Error(e) -> {
      let sz = bit_array.byte_size(e)
      io.println(
        "JSON decode error (expected): " <> int.to_string(sz) <> " bytes",
      )
    }
  }
...
pub fn level1787() -> Nil {
  io.println(
    "--- Level 1787: Parse + Elaborate + Typecheck + Compile + Load full chain ---",
  )
  case parse_string("(lam x x)") {
    Ok(st) -> {
      let su =
        SurfaceUnit(Ref(hash_bytes(bit_array.from_string("full_chain_v17"))), [
...
pub fn level1789() -> Nil {
  ...
  log.info_context(
    "server health",
    dict.from_list([#("readiness", string.inspect(healthy))]),
  )
...
pub fn level1795() -> Nil {
  ...
  io.println(
    ...
    <> to_string(main)
    <> " has .gleam: "
    <> string.inspect(has_ext),
  )
  case parse_string("42") {
    Ok(st) -> {
      let su =
        SurfaceUnit(Ref(hash_bytes(bit_array.from_string("fp_tc_v17"))), [
