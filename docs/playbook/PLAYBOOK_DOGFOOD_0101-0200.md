...
**REPL input:**
```
(define string-starts-with? (lam str (lam prefix (eq? (string-slice str 0 (string-length prefix)) prefix))))
(define string-ends-with? (lam str (lam suffix (let len (string-length str) (let slen (string-length suffix) (eq? (string-slice str (sub len slen) slen) suffix))))))
...
**REPL input:**
```
(list-map (lam x (mul x 2)) (list 1 2 3))
;; Expected: [2,4,6]
(list-filter (lam x (gt? x 1)) (list 1 2 3))
;; Expected: [2,3]
(list-fold (lam acc (lam x (add acc x))) 0 (list 1 2 3))
;; Expected: 6
```

**Expected:** `[2,4,6]`, `[2,3]`, `6`.
