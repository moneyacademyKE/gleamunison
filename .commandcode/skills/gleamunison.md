...
**Version**: 3.7.0
**Test suite**: 53 unit tests, 5674 real dogfood levels (v2-v109, levels 1-6696, 109 batches = 5727 total verifications, 0 failures)
...
scripts/
  ...
  generate_levels.clj   # v2 — 24 templates, per-level imports, --count/--suite
  regenerate_batches.clj# Bulk-regenerate all auto-generated v*.gleam files
src/
...
src/
  ...
  dogfood_v25.gleam      # Batch 25-85: auto-generated (v2 generator: 24 templates, per-level imports)
  dogfood_assert.gleam   # Assertion helpers (assert_eq, assert_prefix)
...
### How Levels Work
Each dogfood level is a public function `pub fn levelN() -> Nil` that exercises a specific runtime capability. Levels are organized by batch (50 levels + 1 cert per batch). There are 109 dogfood files (v2 through v109) covering 5674 real levels. Batches v10, v23-v109 are generated by `scripts/generate_levels.clj` (v2 generator: 24 templates, per-level imports, `--count` and `--suite` flags).
...
**Real levels**: 5674 across 109 dogfood files (v2-v109, levels 1-6696)
**Stub levels**: 0 (all gaps filled by auto-generated batches)
**Unit tests**: 53
**Total verifications**: 5727
**Failures**: 0
**Build warnings**: 1061 (cosmetic, unused imports/vars)
**Generator**: `scripts/generate_levels.clj` — v2, 24 templates, per-level imports, `--count N`, `--suite N`
**Infinite loop**: `scripts/loop_infinite.clj` — v2, zombie cleanup, retry detection, error alert
...
| 1401-1450 | v10 | Phase 9 (regenerated, v2 generator) |
...
| 2271-2320 | v23 | Handle compile, 7/7 pattern variants, guards, full pipeline (regenerated) |
| 2321-2370 | v24 | Cross-module RefTo exec, Handle full pipeline, REPL define+use (regenerated) |
| 2371-6696 | v25-v109 | Auto-generated (v2 generator: 24 templates, per-level imports) |

### Roadmap (15/15 items complete — v3.7.0)

All items from `docs/ROADMAP_ITEMS.md` implemented:
1. **Generator v2**: 24 templates (3 new: Construct, type_pretty, infer_term), per-level imports, `--count N`, `--suite N`, numeric batch sorting
2. **Loop v2**: Zombie cleanup (pkill before spawn), retry detection (exit 1 after 3 fails), concise prompt
3. **Infrastructure**: Regenerated all batches (warnings 1244→1061), removed orphaned scripts, restored hand-crafted files, assertion helpers
...
| 7 | 1061 build warnings — all unused imports/vars (cosmetic, per-level imports reduced from 1244) | all batches |
| 8 | `cmd -p` requires `--yolo` for file write and shell permissions in auto loop | loop_infinite.clj |
| 9 | Prompt must be purely imperative — "analyze" triggers exploration deadlock | loop_infinite.clj |
| 10 | Numeric string-sort bug: `v9` sorts after `v85` alphabetically (fixed in generator v2) | generate_levels.clj |
